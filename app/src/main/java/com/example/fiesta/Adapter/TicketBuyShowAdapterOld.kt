...
class TicketBuyShowAdapterOld(val Parent: Activity, val days: Int, val showArray: JSONArray, val eventStart: Date, val realTime: String, val click:(showId: String?, showName: String?, detail: String?) -> Unit): RecyclerView.Adapter<TicketBuyShowAdapterOld.ViewHolder>() {
        ...
        if(showId.isNotEmpty()){
            ...
            if(date1.parse(showtime[0])<date2.parse(realTime) && date1.parse(showtime[0]).day != date2.parse(realTime).day){
                for (i in 0..showname.size-1){
                    holder.mDataList.add(TimeLineModel("${showname[i]}", "${showId[i]}", "${showtime[i]}","${showdetail[i]}", OrderStatus.COMPLETED,false))
                }
                    ...
                    for (i in 0..showname.size-1){
                        if (date1.parse(showtime[i]) < temp) {
                            if(i != showtime.size-1 && date1.parse(showtime[i+1]) > temp){
                                holder.mDataList.add(TimeLineModel("${showname[i]}", "${showId[i]}", "${showtime[i]}","${showdetail[i]}", OrderStatus.ACTIVE,false))
                            }else{
                                holder.mDataList.add(TimeLineModel("${showname[i]}", "${showId[i]}", "${showtime[i]}","${showdetail[i]}", OrderStatus.COMPLETED,false))
                            }
                        } else {
